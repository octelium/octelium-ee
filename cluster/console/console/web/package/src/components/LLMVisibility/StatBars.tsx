  ...
  const renderRow = (
    ...
    const inner = (
      <>
        <span
          aria-hidden="true"
          className="absolute inset-y-0 left-0 rounded-lg opacity-[0.12] transition-[width] duration-200 ease-out"
        ...
        <span className="relative flex min-w-0 flex-1 items-center gap-2.5">
          ...
          <span
            className={twMerge(
              ...
              isOther ? "text-slate-500" : "text-slate-700",
            )}
          >
            {label}
          </span>
        ...
        <span className="relative flex shrink-0 items-baseline gap-2.5">
          ...
          <span className="w-11 text-right text-micro font-normal tabular-nums text-slate-500">
            {share.toFixed(1)}%
          </span>
        </span>
      </>
    ...
    const className =
      "group relative flex min-h-9 w-full items-center gap-3 overflow-hidden rounded-lg border border-transparent px-2.5 py-1.5 transition-[border-color,background-color] duration-200 hover:border-slate-200 hover:bg-slate-50/60";
    ...
    <div className="flex w-full flex-col gap-0.5">
      ...
      {rest.length > 0 && (
        <button
          ...
          className="mt-1.5 flex w-full cursor-pointer items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-normal text-slate-500 outline-none transition-colors duration-200 hover:bg-slate-50 hover:text-slate-800"
        >
      ...
      {props.totalCount !== undefined &&
        props.totalCount > props.items.length && (
          <p className="mt-1 px-2.5 text-micro font-normal text-slate-500">
