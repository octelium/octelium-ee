...
const SchemaProperty = (props: {
  ...
  return (
    <div className={depth > 0 ? "ml-3 border-l border-slate-200 pl-3" : ""}>
      <button
        type="button"
        className="flex w-full min-w-0 items-center gap-2 rounded-lg px-2.5 py-2 text-left transition-colors duration-200 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400"
      ...
      {(props.schema.description || props.schema.format || defaultValue) && (
        <div className="ml-10 flex flex-wrap items-center gap-x-2 gap-y-1 pb-2 text-micro font-medium text-slate-500">
      ...
      {props.schema.enum && props.schema.enum.length > 0 && (
        <div className="ml-10 mb-2 flex flex-wrap gap-1">
          {props.schema.enum.slice(0, 12).map((value) => (
            <span
              key={String(value)}
              className="rounded-md border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-micro font-normal text-slate-500"
            >
              {String(value)}
            </span>
          ))}
          {props.schema.enum.length > 12 && (
            <span className="px-1 py-0.5 text-micro font-normal text-slate-500">
      ...
      <Drawer
        ...
        title={
          <div className="flex min-w-0 items-center gap-2">
            <FileJson2 size={15} className="shrink-0 text-slate-500" />
