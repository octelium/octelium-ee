...
const Field = (props: {
  ...
  <div className={props.wide ? "sm:col-span-2" : ""}>
    <div className="mb-1 text-micro font-semibold uppercase tracking-[0.07em] text-slate-500">
      {props.label}
    </div>
...
export default () => {
    ...
    return (
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-5 px-2 py-2 sm:px-4 sm:py-4">
        <section className="flex min-h-[360px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white/70 px-6 text-center">
          <CircleAlert size={22} className="mb-3 text-slate-500" />
  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-5 px-2 py-2 sm:px-4 sm:py-4">
      <header className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white px-4 py-4 shadow-card sm:flex-row sm:items-center sm:justify-between sm:px-5">
        ...
        <Link
          ...
          preventScrollReset
          className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition-colors duration-200 hover:border-slate-400 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500"
        >
          ...
          <Field label="Current session" wide>
            {sessionName ? (
              <Link
                ...
                state={{ returnTo: "/settings" }}
                preventScrollReset
                className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition-colors duration-200 hover:border-slate-400 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500"
              >
            ...
            <Field label="Tags" wide>
              <div className="flex flex-wrap gap-1.5">
                {metadata.tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-md border border-slate-200 bg-slate-100 px-2 py-1 text-xs font-normal text-slate-600"
                  >
                    {tag}
                  </span>
                ))}
