  ...
  return (
    <div className="space-y-5">
      {status && <section className="rounded-xl border border-slate-200 bg-white p-4"><div className="mb-3 flex items-center gap-2 text-sm font-bold text-slate-800"><Network size={15} /> Cluster status</div><div className="grid gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-3 [&>div>:last-child]:font-bold">
        {status.domain && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Domain</div><CopyText value={status.domain} /></div>}
        {mode && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Network mode</div><div className="mt-1 text-sm font-semibold text-slate-700">{mode.replaceAll("_", " ")}</div></div>}
        {network?.clusterNetwork?.v4 && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Cluster IPv4 network</div><CopyText value={network.clusterNetwork.v4} /></div>}
        {network?.clusterNetwork?.v6 && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Cluster IPv6 network</div><CopyText value={network.clusterNetwork.v6} /></div>}
        {network?.serviceSubnet?.v4 && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Service IPv4 subnet</div><CopyText value={network.serviceSubnet.v4} /></div>}
        {network?.serviceSubnet?.v6 && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Service IPv6 subnet</div><CopyText value={network.serviceSubnet.v6} /></div>}
        {network?.wgConnSubnet?.v4 && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">WireGuard connection subnet</div><CopyText value={network.wgConnSubnet.v4} /></div>}
        {network?.quicConnSubnet?.v4 && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">QUIC connection subnet</div><CopyText value={network.quicConnSubnet.v4} /></div>}
        {networkConfig?.v4?.clusterNetwork && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Configured IPv4 network</div><CopyText value={networkConfig.v4.clusterNetwork} /></div>}
        {networkConfig?.v6?.clusterNetwork && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Configured IPv6 network</div><CopyText value={networkConfig.v6.clusterNetwork} /></div>}
        {networkConfig?.wireguard && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">WireGuard</div><div className="mt-1 text-sm font-semibold text-slate-700">Port {networkConfig.wireguard.gatewayPort || "default"} · MTU {networkConfig.wireguard.mtu || "default"}</div></div>}
        {networkConfig?.quicv0 && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">QUICv0</div><div className="mt-1 text-sm font-semibold text-slate-700">{networkConfig.quicv0.enable ? "Enabled" : "Disabled"} · Port {networkConfig.quicv0.gatewayPort || "default"}</div></div>}
        {status.secretManager?.address && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Secret manager</div><div className="flex flex-wrap items-center gap-2"><CopyText value={status.secretManager.address} /><span className="text-xs font-normal text-slate-500">{status.secretManager.tls ? "TLS" : "No TLS"}</span></div></div>}
        {!!status.device?.probes.length && <div><div className="text-micro font-semibold uppercase tracking-wide text-slate-500">Device probes</div><div className="mt-1 text-sm font-semibold text-slate-700">{status.device.probes.length.toLocaleString()}</div></div>}
      </div></section>}
