...
const TimestampPicker = ({
  ...
  const chooseRelativeTime = (amount: number, unit: RelativeUnit) => {
  ...
  return (
    <Input.Wrapper label={label} description={description} className="w-full">
      ...
      >
        ...
        <Popover.Dropdown
          p={0}
          style={{ maxWidth: "calc(100vw - 24px)", overflow: "hidden" }}
        >
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                    {PRESETS.map((preset) => (
                      <Button
                        ...
                        type="button"
                        variant="default"
                        onClick={() => chooseRelativeTime(preset.value, preset.unit)}
                  ...
                  <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_170px]">
                    <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-50/50 p-1">
                      <DatePicker
                        value={selectedDate}
                        onChange={(nextDate) => {
                          setSelectedDate(nextDate ? dayjs(nextDate).toDate() : null);
                          setValidationError(undefined);
                        }}
                        minDate={
                          isFuture && !disableExcludePast ? new Date() : undefined
                        }
                        styles={{
                    ...
                    <div className="flex min-w-0 flex-col gap-3">
                      <TimeInput
                        label="Time"
                        value={timeValue}
                        leftSection={<Clock3 size={12} className="text-slate-400" />}
                        onChange={(event) => {
