...
func TestCoreSummaryCountsComprehensive(t *testing.T) {
...

	{
		...
		assert.Nil(t, err, "%+v", err)
		assert.Equal(t, uint64(3), resp.TotalNumber)
		assert.Equal(t, uint64(2), resp.TotalHuman)
		assert.Equal(t, uint64(1), resp.TotalWorkload)
		assert.Equal(t, uint64(1), resp.TotalDisabled)
	}

	{
		...
		assert.Equal(t, uint64(2), resp.TotalClient)
		assert.Equal(t, uint64(2), resp.TotalClientless)
		assert.Equal(t, uint64(1), resp.TotalConnected)
		assert.Equal(t, uint64(2), resp.TotalUser)
		assert.Equal(t, uint64(1), resp.TotalDevice)
		assert.Equal(t, uint64(1), resp.TotalClientlessBrowser)
		assert.Equal(t, uint64(2), resp.TotalActive)
		assert.Equal(t, uint64(1), resp.TotalPending)
		assert.Equal(t, uint64(1), resp.TotalRejected)
	}

	{
		resp, err := env.srv.getSummaryCoreService(env.ctx, &vcorev1.GetServiceSummaryRequest{})
		assert.Nil(t, err, "%+v", err)
		assert.Equal(t, uint64(14), resp.TotalNumber)
		assert.Equal(t, uint64(3), resp.TotalPublic)
		assert.Equal(t, uint64(1), resp.TotalAnonymous)
		assert.Equal(t, uint64(1), resp.TotalTCP)
		assert.Equal(t, uint64(1), resp.TotalUDP)
		assert.Equal(t, uint64(1), resp.TotalHTTP)
		assert.Equal(t, uint64(1), resp.TotalSSH)
		assert.Equal(t, uint64(1), resp.TotalKubernetes)
		assert.Equal(t, uint64(1), resp.TotalPostgres)
		assert.Equal(t, uint64(1), resp.TotalMysql)
		assert.Equal(t, uint64(1), resp.TotalDNS)
		assert.Equal(t, uint64(1), resp.TotalGRPC)
		assert.Equal(t, uint64(1), resp.TotalWeb)
		assert.Equal(t, uint64(1), resp.TotalSOCKS5)
		assert.Equal(t, uint64(1), resp.TotalRDPWeb)
		assert.Equal(t, uint64(1), resp.TotalMCP)
		assert.Equal(t, uint64(1), resp.TotalLLM)
		assert.Equal(t, uint64(1), resp.TotalDisabled)
	}

	{
		...
		assert.Nil(t, err, "%+v", err)
		assert.Equal(t, uint64(2), resp.TotalNumber)
		assert.Equal(t, uint64(1), resp.TotalDisabled)
		assert.Equal(t, uint64(4), resp.TotalRule)
		assert.Equal(t, uint64(2), resp.TotalRuleAllow)
		assert.Equal(t, uint64(2), resp.TotalRuleDenied)
	}

	{
		...
		assert.Equal(t, uint64(2), resp.TotalUser)
		assert.Equal(t, uint64(1), resp.TotalDisabled)
		assert.Equal(t, uint64(1), resp.TotalAuthenticationToken)
		assert.Equal(t, uint64(1), resp.TotalOAuth2)
		assert.Equal(t, uint64(1), resp.TotalAccessToken)
	}

	{
		...
		assert.Equal(t, uint64(4), resp.TotalNumber)
		assert.Equal(t, uint64(1), resp.TotalDisabled)
		assert.Equal(t, uint64(1), resp.TotalGithub)
		assert.Equal(t, uint64(1), resp.TotalOIDC)
		assert.Equal(t, uint64(1), resp.TotalSAML)
		assert.Equal(t, uint64(1), resp.TotalOIDCIdentityToken)
	}

	{
		...
		assert.Equal(t, uint64(2), resp.TotalUser)
		assert.Equal(t, uint64(3), resp.TotalActive)
		assert.Equal(t, uint64(1), resp.TotalPending)
		assert.Equal(t, uint64(1), resp.TotalRejected)
		assert.Equal(t, uint64(1), resp.TotalLinux)
		assert.Equal(t, uint64(1), resp.TotalWindows)
		assert.Equal(t, uint64(1), resp.TotalMac)
		assert.Equal(t, uint64(1), resp.TotalAndroid)
		assert.Equal(t, uint64(1), resp.TotalIOS)
	}

	{
		...
		assert.Nil(t, err, "%+v", err)
		assert.Equal(t, uint64(4), resp.TotalNumber)
		assert.Equal(t, uint64(2), resp.TotalFIDO)
		assert.Equal(t, uint64(1), resp.TotalTOTP)
		assert.Equal(t, uint64(1), resp.TotalTPM)
		assert.Equal(t, uint64(2), resp.TotalUser)
		assert.Equal(t, uint64(1), resp.TotalDevice)
		assert.Equal(t, uint64(2), resp.TotalActive)
		assert.Equal(t, uint64(1), resp.TotalPending)
		assert.Equal(t, uint64(1), resp.TotalRejected)
		assert.Equal(t, uint64(1), resp.TotalFIDOPlatform)
		assert.Equal(t, uint64(1), resp.TotalFIDORoaming)
		assert.Equal(t, uint64(1), resp.TotalFIDOIsPasskey)
		assert.Equal(t, uint64(1), resp.TotalFIDOIsHardware)
	}
